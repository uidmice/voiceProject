...
def timer_callback(idx):
    global status
    status = 0
    process = subprocess.Popen(['python /home/pi/voiceProject/Piwho/recognizer.py tem' + str(idx) + ".wav" ], cwd="./Piwho", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    stdout, stderr = process.communicate()
    name = stdout.decode('utf-8').rstrip().capitalize()
    print("Sorry, I didn\'t get it. Are you " + name + "?")
